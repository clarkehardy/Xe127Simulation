...
import argparse
import time
from ComputeGammaObservables import ComputeObservedLight
from ComputeGammaObservables import ComputeObservedCharge
...
#ReconFile = input_file.split('.root')[0]+'_recon.root'
#SensReconFile = input_file.splot('.root')[0]+'_sensrecon.root'
...
reduced_df = output_df[output_df['fNTE']>0]

print('{} events with nonzero charge produced.'.format(len(reduced_df.index)))

print('Saving output dataframe...')

reduced_df.to_pickle(output_dir + filename + '_REDUCED.pkl',compression='gzip')

print('Finished processing {} at {:5.5}s'.format(filename,time.time()-start_time))
