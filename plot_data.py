...
def make_figs(tpc,lm_nn,data,cuts,path,name,rlim,zlim,peak_sep):
    ...
    hist,r_edges,z_edges = np.histogram2d(data.weighted_radius.values[cuts],data.z.values[cuts],bins=50,density=True)
    r_bins = (r_edges[:-1]+r_edges[1:])/2.
    z_bins = (z_edges[:-1]+z_edges[1:])/2.
    R,Z = np.meshgrid(r_bins,z_bins)
    im = ax.imshow(hist.T/(2*np.pi*R),extent=[r_edges.min(),r_edges.max(),z_edges.min(),z_edges.max()])
    ...
    hist,x_edges,y_edges = np.histogram2d(data.weighted_x.values[cuts],data.weighted_y.values[cuts],bins=50,density=True)
    x_bins = (x_edges[:-1]+x_edges[1:])/2.
    y_bins = (y_edges[:-1]+y_edges[1:])/2.
    X,Y = np.meshgrid(x_bins,y_bins)
    im = ax.imshow(hist.T/(tpc.zmax-tpc.zmin),extent=[x_edges.min(),x_edges.max(),y_edges.min(),y_edges.max()])
    ...
    plt.figure(figsize=(4,3))
    plt.hist(data.evt_charge_including_noise.values[cuts],bins=200,color='blue',label='Observed charge',histtype=u'step')
    ...
    plt.figure(figsize=(4,3))
    plt.hist2d(data.fInitNOP.values[cuts],data['Observed Light'][cuts],bins=200,norm=mpl.colors.LogNorm())
    ...
    plt.figure(figsize=(4,3))
    plt.hist([data['eff'][data['peak']==1],data['eff'][data['peak']==2],data['eff']],bins=200,\
...
def plot_results(tpc,lm_nn,lm_again,rlim,zlim,path,name):
    ...
    plt.figure(figsize=(4,3))
    plt.hist(np.ndarray.flatten(h_nn.values/h_again.values),bins=100,range=(0.8,1.2),color='navy',histtype=u'step')
